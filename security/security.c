...
/**
 * security_lsm_set_self_policy() - Manage caller's LSM policies
 ...
 * Return: Returns 0 on success, error on failure.
 */
int security_lsm_set_self_policy(u32 lsm_id, u32 op, void __user *buf,
				 size_t size, u32 flags)
{
	int rc = LSM_RET_DEFAULT(lsm_set_self_policy);
	struct lsm_static_call *scall;

	if (size > (CONFIG_LSM_SET_SELF_POLICY_MAX_BUFFER_SIZE))
		return -E2BIG;

	lsm_for_each_hook(scall, lsm_set_self_policy) {
		if ((scall->hl->lsmid->id) == lsm_id) {
			rc = scall->hl->hook.lsm_set_self_policy(lsm_id, op, buf, size, flags);
			break;
		}
	}
...
EXPORT_SYMBOL(security_lsm_set_self_policy);

/**
 * security_lsm_set_system_policy() - Manage system LSM policies
 * @lsm_id: id of the lsm to target
 * @op: Operation to perform (one of the LSM_POLICY_XXX values)
 * @buf: userspace pointer to policy data
 * @size: size of @buf
 ...
 * Return: Returns 0 on success, error on failure.
 */
int security_lsm_set_system_policy(u32 lsm_id, u32 op, void __user *buf,
				   size_t size, u32 flags)
{
	int rc = LSM_RET_DEFAULT(lsm_set_system_policy);
	struct lsm_static_call *scall;

	if (size > (CONFIG_LSM_SET_SYSTEM_POLICY_MAX_BUFFER_SIZE))
		return -E2BIG;

	lsm_for_each_hook(scall, lsm_set_system_policy) {
		if ((scall->hl->lsmid->id) == lsm_id) {
			rc = scall->hl->hook.lsm_set_system_policy(lsm_id, op, buf, size, flags);
			break;
		}
	}
...
EXPORT_SYMBOL(security_lsm_set_system_policy);

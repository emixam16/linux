					  ...
					  u32 flags)
{
	...
	if (get_user(fmt, (uint8_t __user *)buf))
		return -EFAULT;

	if (fmt > SMK_CHANGE_FMT)
		return -EINVAL;

	error = smk_write_rules_list(NULL, buf + 1, size - 1, &pos, NULL, NULL,
				     fmt);
					...
					u32 flags)
{
	...
	tsp = smack_cred(current_cred());
	error = smk_write_rules_list(NULL, buf + 1, size - 1, &pos,
				     &tsp->smk_rules, &tsp->smk_rules_lock,
				     fmt);

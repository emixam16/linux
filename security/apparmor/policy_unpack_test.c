...
/*
 * unpack_policyns_block() tests. The "policyns" struct is attacker-influenced
 * wire input, so each malformed shape must fail closed (-EPROTO) with e->pos
...
static void policy_unpack_test_policyns_value_over_int_max(struct kunit *test)
{
	...
	/* the parser bounds caps at INT_MAX; larger values fail closed */
...
static void policy_unpack_test_policyns_unterminated_name(struct kunit *test)
{
	...
	/* a :NAME: string chunk without the trailing NUL must fail closed */
	s.target = AA_POLICYNS_TGT_NAME;
	s.structend = false;
	pn_put_block(test, b, &s);
	pn_put_name(test, b, "name");
